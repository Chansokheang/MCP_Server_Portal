...
const I18N = (() => {
  ...
  Object.assign(dict, {
    ...
    "Asked from the model": "모델에게 물음", "Instructions for the model": "모델을 위한 지침", "no usage notes": "사용 안내 없음", "so the model has only tool names and descriptions to go on; add them on its page.": "모델은 도구 이름과 설명만으로 판단해야 합니다. 백엔드 페이지에서 안내를 추가하세요.",
    "The opening lines a model reads when it connects here: who it is helping and how the backends fit together. Each backend's own usage notes are sent after your text; they are shown below and edited on the backend's page.": "모델이 연결할 때 처음 읽는 문장입니다: 누구를 돕는지, 백엔드들이 어떻게 맞물리는지. 각 백엔드의 사용 안내는 이 글 뒤에 이어서 전송되며, 아래에 표시되고 백엔드 페이지에서 편집합니다.", "What the model is told": "모델이 받는 안내", "Preview as": "미리보기 대상",
