...
const I18N = (() => {
  ...
  Object.assign(dict, {
    "What to prepare": "준비할 것", "a value I type in (hidden from the model)": "직접 입력한 값 (모델에게 숨김)", "a default the model may change": "모델이 바꿀 수 있는 기본값", "Values for this gateway": "이 게이트웨이의 값", "Fixed values and defaults": "고정값과 기본값",
    ...
    "A backend is shared by every team, so the values typed on its page are only the starting point. Here you set them for the people who use this gateway, or switch them off. A fixed value is sent on every call and hidden from the model; a default is shown to the model and used only when it leaves the parameter out. Leave the value empty to keep the backend's.": "백엔드는 모든 팀이 함께 쓰므로 백엔드 페이지에 입력한 값은 출발점일 뿐입니다. 여기서 이 게이트웨이 사용자를 위한 값을 정하거나 끌 수 있습니다. 고정값은 매 호출에 보내지고 모델에게 숨겨지며, 기본값은 모델에게 보이고 모델이 매개변수를 생략했을 때만 쓰입니다. 값을 비워 두면 백엔드의 값을 그대로 씁니다.",
    "Applies on the next connection. The backend's own values stay as they are for every other gateway.": "다음 연결부터 적용됩니다. 백엔드 자체의 값은 다른 게이트웨이에서 그대로 유지됩니다.", "the default value": "기본값", "Default": "기본값", "Fixed value": "고정값", "the value to send": "보낼 값", "field, e.g. id": "필드, 예: id", "Filled in from the signed-in user": "로그인한 사용자 정보로 자동 입력",
