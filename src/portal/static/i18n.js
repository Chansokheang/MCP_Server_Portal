...
const I18N = (() => {
  ...
  Object.assign(dict, {
    "What to prepare": "준비할 것", "Values for this gateway": "이 게이트웨이의 값", "Fixed values and defaults": "고정값과 기본값",
    "fixed (hidden)": "고정값 (숨김)", "default (may change)": "기본값 (변경 가능)", "Value here": "여기서의 값", "Kind": "종류", "On": "사용",
    "Changed here": "여기서 변경됨", "From the backend": "백엔드에서 상속", "This gateway only": "이 게이트웨이만", "Use the backend's": "백엔드 값 사용", "Save values": "값 저장", "Add a value for this gateway": "이 게이트웨이용 값 추가", "every tool that takes it": "이 매개변수를 받는 모든 도구",
    "Pick the backend, the tool and the parameter, type the value, and switch it off whenever it should not apply. Choose \"every tool that takes it\" for something like an API token header, so it is set once. A row for one tool wins over an every-tool row.": "백엔드, 도구, 매개변수를 고르고 값을 입력하세요. 적용하지 않을 때는 끄면 됩니다. API 토큰 헤더처럼 한 번만 정하면 되는 값은 \"이 매개변수를 받는 모든 도구\"를 고르세요. 특정 도구의 행이 모든 도구 행보다 우선합니다.", "the value": "값",
    "A value that belongs to the people who use this gateway: their project, their team's usual choice. A fixed value is sent on every call and hidden from the model; a default is shown to the model and used only when it leaves the parameter out. Pick the backend, the tool and the parameter, type the value, and switch it off whenever it should not apply.": "이 게이트웨이 사용자에게 속한 값입니다: 그들의 프로젝트, 팀에서 늘 쓰는 선택. 고정값은 매 호출에 보내지고 모델에게 숨겨지며, 기본값은 모델에게 보이고 모델이 매개변수를 생략했을 때만 쓰입니다. 백엔드, 도구, 매개변수를 고르고 값을 입력하세요. 적용하지 않을 때는 끄면 됩니다.",
    "Applies on the next connection. Other gateways are not affected.": "다음 연결부터 적용됩니다. 다른 게이트웨이에는 영향이 없습니다.", "Remove": "제거",
    "This is a fact about the API, so it holds on every gateway: the gateway writes it into the tool's description and, when a call arrives without the value, tells the model what to call first. Rows marked seen were observed in real calls: a value one tool returned was used by another. A fixed value or a default for a parameter depends on who is using it, so those are set on each gateway's page.": "이는 API 자체의 사실이므로 모든 게이트웨이에 적용됩니다. 게이트웨이가 이를 도구 설명에 적어 넣고, 값 없이 호출이 오면 모델에게 먼저 무엇을 호출할지 알려 줍니다. '관찰됨' 행은 실제 호출에서 한 도구가 돌려준 값이 다른 도구에 쓰인 경우입니다. 매개변수의 고정값이나 기본값은 누가 쓰느냐에 따라 다르므로 각 게이트웨이 페이지에서 설정합니다.", "the default value": "기본값", "Default": "기본값", "Fixed value": "고정값", "the value to send": "보낼 값", "field, e.g. id": "필드, 예: id", "Filled in from the signed-in user": "로그인한 사용자 정보로 자동 입력",
    ...
    "Asked from the model": "모델에게 물음", "Instructions for the model": "모델을 위한 지침", "no usage notes": "사용 안내 없음", "usage notes follow": "사용 안내가 이어짐",
    "The opening lines a model reads when it connects here: who it is helping and how the backends fit together. Each backend's own usage notes are sent after your text; edit them on the backend's page.": "모델이 연결할 때 처음 읽는 문장입니다: 누구를 돕는지, 백엔드들이 어떻게 맞물리는지. 각 백엔드의 사용 안내는 이 글 뒤에 이어서 전송되며, 백엔드 페이지에서 편집합니다.", "What the model is told": "모델이 받는 안내", "Preview as": "미리보기 대상",
